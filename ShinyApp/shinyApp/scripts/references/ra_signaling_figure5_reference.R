# ---------------------------------------------------------------------------
# Retinoic acid signaling reference figures
# ---------------------------------------------------------------------------
# This reference-only script reproduces publication-style RA signaling figures
# from a full/specific Seurat object and writes PDFs into the app www/ folder.
# It is not sourced by ui.R or server.R.
...
# Resolve this script's directory across Rscript and source(...) execution.
...
app_dir_candidates <- c(
  normalizePath(file.path(script_dir, "..", ".."), mustWork = FALSE),
  normalizePath(file.path(getwd(), "ShinyApp", "shinyApp"), mustWork = FALSE),
  normalizePath(getwd(), mustWork = FALSE)
)
app_dir <- app_dir_candidates[
  file.exists(file.path(app_dir_candidates, "app_support.R"))
][1]
if (is.na(app_dir) || !nzchar(app_dir)) {
  stop("Could not locate the app root for this reference script.", call. = FALSE)
}
source(file.path(app_dir, "app_support.R"))
app_dir <- sc_set_app_dir(app_dir)

# Reference outputs intentionally go to www/ because the historical PDFs were
# consumed as app/static artifacts during figure development.
output_dir <- sc_www_path(app_dir = app_dir)
dir.create(output_dir, showWarnings = FALSE, recursive = TRUE)

# Prefer Data/ Seurat inputs after the data-root migration, with legacy
# candidates supplied by sc_data_candidates().
input_candidates <- sc_resolve_paths(sc_data_candidates(
  c(
    "specificCellID_Original.rds",
...
if (is.na(input_path) || !nzchar(input_path)) {
  stop(
    "Could not find a specificCellID input file. Checked: ",
...
# Figure 5a dotplot
# ---------------------------------------------------------------------------
# Curated RA pathway genes used for the reference dotplot.
...
# Publication-style diverging palette for scaled average expression.
RdBu <- rev(colorRampPalette(RColorBrewer::brewer.pal(9, "RdBu"))(100))

RA_dp <- DotPlot(obj_ra_dotplot,
  features = ra_genes_to_plot, dot.scale = 5.5, assay = NULL,
  col.min = -2.5,
  col.max = 2.5,
  dot.min = 0,
  idents = NULL,
...
# ---------------------------------------------------------------------------
# Figure 5c stage trajectory lineplots
# ---------------------------------------------------------------------------
library(Seurat)
library(reshape2)
library(dplyr)
...
obj_ra_lineplot <- subset(obj_ra_lineplot, idents = c(
  "Aund", "A1-2", "A3-4", "Ain", "Type B", "ePL", "lPL", "L", "L/Z",
...
# Build one row of stage-looped line plots by averaging expression within
# sample/generalCellID groups and z-scoring each gene inside each cell type.
make_rescaled_looped_plot <- function(obj, genes_to_plot, title_label) {
  ...
  # Plot
  return(ggplot(df_rescaled, aes(x = sample, y = scaled_expr, color = gene, group = gene)) +
...
# Gene colors for each row of the reference stage-trajectory figure. The second
# set below intentionally overrides the earlier draft colors.
# For Stra8 and Stra6
...
# Generate each pathway row, then strip axes/strips to match the compact
# reference figure layout.
stra68 <- make_rescaled_looped_plot(obj_ra_lineplot,
  genes_to_plot = c("Stra8", "Stra6"),
  title_label = "Stra Genes Across Stages (Z-Scored)"
) + scale_color_manual(values = stra_colors)

aldh1a123 <- make_rescaled_looped_plot(obj_ra_lineplot,
  genes_to_plot = c("Aldh1a1", "Aldh1a2", "Aldh1a3"),
  title_label = "Aldh1a Genes Across Stages (Z-Scored)"
) + scale_color_manual(values = aldh_colors)

cyp26abc1 <- make_rescaled_looped_plot(obj_ra_lineplot,
  genes_to_plot = c("Cyp26a1", "Cyp26b1", "Cyp26c1"),
  title_label = "Cyp26 Genes Across Stages (Z-Scored)"
) + scale_color_manual(values = cyp26_colors)
...
# Stack the three pathway rows and export the composed reference panel.
