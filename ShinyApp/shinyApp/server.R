# ---------------------------------------------------------------------------
# SpermInteractive Shiny server
# ---------------------------------------------------------------------------
# This file contains the server-side runtime for the SpermInteractive app. The
# ShinyCell dataset tabs, precomputed interactive figures, share-link handling,
# modal/export workflows, and specialized spermatogenesis/RA/CCC views are kept
# together because their UI IDs are generated across ui.R and
# dataset_tab_builders.R.
#
# Comments in this file emphasize intent, data ownership, and reactive control
# flow. They avoid restating individual assignments unless the behavior is easy
# to break during maintenance.

# ---------------------------------------------------------------------------
# Package imports and shared app helpers
# ---------------------------------------------------------------------------
library(Seurat)
library(ShinyCell)
library(shiny)
...
sc_source("dataset_tab_builders.R")

# Load display-name and metadata corrections before any ShinyCell configuration
# object is materialized. These rules are applied lazily when each dataset is
# first opened.
metadata_rules <- get_metadata_overrides(base_dir = sc_www_path(app_dir = app_dir))

# Create a ShinyCell-aware lazy RDS loader. The returned function reads the
# first available asset path once, caches it, and reuses the object afterward.
lazy_rds_loader <- function(paths) {
  if (length(paths) == 0) {
    stop("No paths provided to loader.", call. = FALSE)
  }
  return(sc_lazy_loader(
    paths,
...
renderPlot <- function(expr, ..., bg = "transparent") {
  ...
  env <- parent.frame()
  return(shiny::renderPlot(expr, ..., bg = bg, quoted = TRUE, env = env))
}

sc_is_truthy <- function(value) {
  candidate <- tolower(trimws(as.character(value)[1]))
  return(nzchar(candidate) && candidate %in% c("1", "true", "t", "yes", "y", "on"))
}

# Enable lightweight lag profiling from either an R option or an environment
# variable. Normal app sessions skip the profiling overhead.
sc_profile_enabled <- function() {
  return(isTRUE(getOption("sperminteractive.profile_lag", FALSE)) ||
    sc_is_truthy(Sys.getenv("SPERMINTERACTIVE_PROFILE_LAG", unset = "")))
}

# Profiling state is stack-based so nested plot helpers can attribute timing to
# the currently-rendering plot without passing a context object everywhere.
...
# Remove NULL values before metadata is serialized to compact JSON log records.
sc_profile_compact <- function(values) {
  keep <- vapply(values, function(value) !is.null(value), logical(1))
  return(values[keep])
}

# Return the active profiling context, if profiling is currently inside a plot.
...
# Start a profiling context for one plot render and seed standard timing fields.
...
# Remove the active profiling context after a profiled render completes.
...
# Attach metadata to the active profiling context. Calls outside a profiled
# render are intentionally no-ops so helpers can call this unconditionally.
...
# Add elapsed time or count-like values to the active profiling context.
...
# Convenience wrapper for incrementing count metrics.
sc_profile_increment_metric <- function(name, amount = 1L) {
  return(sc_profile_add_metric(name, amount))
}

# Evaluate an expression and attribute its elapsed time to a named metric when
# lag profiling is enabled.
...
# Emit a single JSON line for each profiled plot render.
...
# Wrap a plot expression with profiling setup and error-aware teardown.
...
# ---------------------------------------------------------------------------
# Precomputed interactive assets
# ---------------------------------------------------------------------------

# Load CCC heatmap data (Figure 6D). This CSV is small enough to keep eager so
# default ligand-receptor selections can be seeded immediately.
...
# Precomputed, minimal assets generated by scripts/build/build_interactive_assets.R.
get_spg_avg_expr <- lazy_rds_loader(sc_data_candidates("spg_avg_expr_by_button.rds"))
get_ra_dot_avg <- lazy_rds_loader(sc_data_candidates("ra_dot_avg_expr.rds"))
get_ra_dot_pct <- lazy_rds_loader(sc_data_candidates("ra_dot_pct_expr.rds"))
get_ra_line_mean <- lazy_rds_loader(sc_data_candidates("ra_line_mean_expr.rds"))
get_interactive_genes <- lazy_rds_loader(sc_data_candidates("interactive_genes.rds"))
...
dev_check_ra_line <- function(
    ...
    specific_paths = sc_data_candidates(c("specificCellID_slim.rds", "specificCellID.rds"))) {
  if (!isTRUE(enable)) {
    return(invisible(NULL))
  }
...
# Create read-only active bindings for ShinyCell data assets. Each binding reads
# its RDS file on first access and then serves the cached object for the rest of
# the R process.
make_lazy_data <- function(name, path, postprocess = identity, env = parent.frame()) {
  target_env <- env
  return(makeActiveBinding(
    sym = name,
    fun = local({
      cache <- NULL
      function(value) {
...
# Register the five built-in ShinyCell datasets. Metadata override postprocessing
# runs only after the corresponding configuration/default file is first read.
make_lazy_data("sc3conf", sc_data_candidates("sc3conf.rds"), function(obj) apply_metadata_overrides_to_conf(obj, rules = metadata_rules))
make_lazy_data("sc3def", sc_data_candidates("sc3def.rds"), function(obj) apply_metadata_overrides_to_def(obj, sc3conf, rules = metadata_rules))
make_lazy_data("sc3gene", sc_data_candidates("sc3gene.rds"), normalize_gene_index)
make_lazy_data("sc3meta", sc_data_candidates("sc3meta.rds"))

make_lazy_data("sc4conf", sc_data_candidates("sc4conf.rds"), function(obj) apply_metadata_overrides_to_conf(obj, rules = metadata_rules))
make_lazy_data("sc4def", sc_data_candidates("sc4def.rds"), function(obj) apply_metadata_overrides_to_def(obj, sc4conf, rules = metadata_rules))
make_lazy_data("sc4gene", sc_data_candidates("sc4gene.rds"), normalize_gene_index)
make_lazy_data("sc4meta", sc_data_candidates("sc4meta.rds"))

make_lazy_data("sc5conf", sc_data_candidates("sc5conf.rds"), function(obj) apply_metadata_overrides_to_conf(obj, rules = metadata_rules))
make_lazy_data("sc5def", sc_data_candidates("sc5def.rds"), function(obj) apply_metadata_overrides_to_def(obj, sc5conf, rules = metadata_rules))
make_lazy_data("sc5gene", sc_data_candidates("sc5gene.rds"), normalize_gene_index)
make_lazy_data("sc5meta", sc_data_candidates("sc5meta.rds"))

make_lazy_data("sc6conf", sc_data_candidates("sc6conf.rds"), function(obj) apply_metadata_overrides_to_conf(obj, rules = metadata_rules))
make_lazy_data("sc6def", sc_data_candidates("sc6def.rds"), function(obj) apply_metadata_overrides_to_def(obj, sc6conf, rules = metadata_rules))
make_lazy_data("sc6gene", sc_data_candidates("sc6gene.rds"), normalize_gene_index)
make_lazy_data("sc6meta", sc_data_candidates("sc6meta.rds"))

make_lazy_data("sc7conf", sc_data_candidates("sc7conf.rds"), function(obj) apply_metadata_overrides_to_conf(obj, rules = metadata_rules))
make_lazy_data("sc7def", sc_data_candidates("sc7def.rds"), function(obj) apply_metadata_overrides_to_def(obj, sc7conf, rules = metadata_rules))
make_lazy_data("sc7gene", sc_data_candidates("sc7gene.rds"), normalize_gene_index)
make_lazy_data("sc7meta", sc_data_candidates("sc7meta.rds"))

# Cache open HDF5 datasets for bundled expression matrices. Uploaded user
# matrices are excluded in read_h5_gene() so temporary files are not held open.
...
# Read one gene vector from a ShinyCell HDF5 expression matrix and record the
# read cost when lag profiling is active.
...
# Translate a selected gene name into the integer row index expected by the HDF5
# expression matrix. This fails early with UI-facing messages for invalid input.
...
# Default multi-gene selections used when a dataset has not yet received explicit
# user input.
...
# Pick the default multi-gene expression list for a dataset, falling back to the
# current dataset's first valid genes when the preferred genes are unavailable.
default_multiple_geneexpr_genes <- function(prefix, inpGene, fallback = NULL) {
  ...
  return(gene_names[seq_len(min(5, length(gene_names)))])
}
...
# Colour palettes exposed by the ShinyCell plot controls.
cList <- list(
  c(
...
# Standard plot heights, base text sizes, and label sizes shared by generated
# ShinyCell tabs.
...
# Extract a ggplot legend grob so complex heatmap/bubbleplot layouts can place
# legends independently from their main panel.
...
# Shared ggplot theme used by ShinyCell-derived plots. The dark-theme branch
# mirrors the app theme without changing plotting data.
...
# ---------------------------------------------------------------------------
# Shared ShinyCell plotting helpers
# ---------------------------------------------------------------------------

# Detect when a dimensional-reduction plot is using the UMAP embedding.
is_umap_view <- function(inpConf, inpdrX, inpdrY) {
  dr_ids <- inpConf[UI %in% c(inpdrX, inpdrY)]$ID
  dr_ids <- tolower(dr_ids)
  return(length(dr_ids) == 2 && setequal(dr_ids, c("umap_1", "umap_2")))
}

# Return the first configured UI variable for a dataset. This protects subset
# controls from NULL values while lazy dataset inputs are being initialized.
...
# Summarize the percentage of cells expressing a selected gene within each
# metadata group for the CellInfo vs GeneExpr table.
...
# Blend two expression intensities into a bivariate color channel.
...
# Build the bivariate color legend used by the gene coexpression plot.
...
# Summarize single-gene and double-positive cell counts for the coexpression
...
# Parse manually-entered or uploaded gene names, preserving canonical casing from
# the current dataset and separating missing tokens for user feedback.
scParseGeneVector <- function(inp_values, inpGene) {
  ...
  return(list(valid = valid, missing = missing, input = parsed))
}

# Aggregate many selected genes by a metadata group for the multi-gene dotplot
# and its companion data table.
...
# Render the multi-gene expression summary as a dotplot using average expression
# for color and percent-expressing cells for point size.
scMultiGeneDotPlot <- function(dot_data, inpfsz = "Medium", inpdotsz = 1, dark_theme = FALSE) {
  ...
  return(ggplot(dot_data, aes(x = gene, y = group)) +
    geom_point(
...
# ---------------------------------------------------------------------------
# Session runtime and reactive wiring
# ---------------------------------------------------------------------------
shinyServer(function(input, output, session) {
  # Register helper observers used by generated UI tags and server-side
  # selectize controls.
  observe_helpers()

  # Track whether the heavyweight RA/spermatogenesis assets have been loaded for
  # this R process. These flags prevent repeated RDS reads during one session.
  ...
  # Plot gates keep expensive outputs from rendering before their inputs and
  # assets are prepared.
  ...
  # Open a render gate immediately or after a short delay. The delayed path lets
  # hidden tab panes finish becoming visible before plots size themselves.
  open_ra_gate <- function(name, delay = 0) {
    ...
    return(later::later(function() {
      return(ra_plot_gate[[name]] <- TRUE)
    }, delay = delay))
  }

  # Populate the RA Figure 5A gene selector, preserving a shared-link selection
  # when the URL provided one.
  ...
  # Populate the RA Figure 5A cell-type checkboxes in publication order.
  update_ra_celltype_inputs <- function(cell_types) {
    ...
    ordered_choices <- c(
      desired_cell_types[desired_cell_types %in% cell_types],
    ...
    return(updateCheckboxGroupInput(
      session,
      "ra_cell_types",
      choices = ordered_choices,
      selected = selected
  ...
  # Populate the three RA Figure 5C gene rows with publication defaults.
  ...
  # Populate the spermatogenesis gene search box from the precomputed matrix.
  ...
  # Load the spermatogenesis table matrix and initialize its search choices.
  ...
  # Load RA dotplot matrices and initialize Figure 5A controls.
  ...
  # Load RA lineplot assets and initialize Figure 5C controls.
  ...
  # Some hosted sessions render a hidden tab before the browser finishes sizing
  # it. Retry a few startup renders after the RA tab opens so at least one paint
  # lands with visible dimensions.
  observe({
    attempts_left <- startup_reload_retries$retinoic_acid
    if (attempts_left <= 0L || !identical(input$mainTabs, "retinoic_acid")) {
      return()
    }
  ...
  # Client-side preset copy actions report their status through this input.
  ...
  # Theme helpers centralize the dark-mode branch used by plot builders.
  ...
  with_dark <- function(fun, ...) {
    return(fun(..., dark_theme = is_dark_mode()))
  }

  with_dark_static <- function(fun, ...) {
    return(fun(..., dark_theme = isolate(is_dark_mode())))
  }

  # Inputs included in share links. Transient buttons and modal events are
  # excluded below so copied URLs restore state instead of replaying actions.
  ...
  onBookmarked(function(url) {
    ...
    return(session$sendCustomMessage("copy-to-clipboard", list(text = shared_url)))
  })

  # Dormant upload-dataset implementation retained for future reference. The
  # corresponding UI is currently disabled, so this block remains commented out.
  ...
  # })
  # Home cards and tutorial JavaScript send this event to jump directly to a
  # registered navbar tab.
  ...
  # A single modal lock prevents overlapping Shiny modals when users click
  # download buttons or spermatogenesis cells repeatedly.
  app_modal_open <- reactiveVal(FALSE)

  # Try to acquire the app-wide modal lock.
  begin_app_modal <- function() {
    if (isTRUE(app_modal_open())) {
      return(FALSE)
    }
    app_modal_open(TRUE)
    return(TRUE)
  ...
  # Release the app-wide modal lock without closing a modal.
  end_app_modal <- function() {
    app_modal_open(FALSE)
    return(invisible(NULL))
  }

  # Release the modal lock and remove the active Shiny modal.
  close_app_modal <- function() {
    end_app_modal()
    return(removeModal())
  }

  # Render the shared download modal used by RA, spermatogenesis, and dataset
  # plot exports.
  ...
  # Build a safe download filename from optional user input.
  resolve_download_filename <- function(custom_value, default_stem, extension) {
    ...
    return(paste0(custom_value, extension))
  }

  # Read a numeric export dimension, falling back when the UI value is missing
  # or invalid.
  ...
  # Save publication-quality vector output using cairo so text and transparent
  # backgrounds render consistently on the hosted system.
  ...
  # Save raster output with a transparent background for theme compatibility.
  ...
  # Wire one download-open button to the shared modal template.
  bind_single_download_modal <- function(button_id,
    ...
    return(observeEvent(input[[button_id]],
      {
  ...
  # Bind the high-level UMAP overview and stage-split figures for a ShinyCell
  # dataset prefix.
  bind_main_figures <- function(prefix, get_conf, get_meta) {
    make_id <- function(part) paste0(prefix, "mf", part)

    # Resolve the corrected cell-type metadata field used by the main figures.
    get_target_ui <- function() {
      return(resolve_ui_from_id(
        get_conf(),
    ...
    # Return all groups available for the main figure subset checkboxes.
    get_groups <- function() {
      conf <- get_conf()
      ...
      return(strsplit(vals, "\\|")[[1]])
    }

    download_name <- function(name_id, default_stem, extension) {
      return(resolve_download_filename(input[[make_id(name_id)]], default_stem, extension))
    }
    ...
    # Download handlers rebuild plots from isolated theme state so the exported
    # file matches the visible theme without adding reactive dependencies.
    ...
    return(observeEvent(input[[make_id("downloads_open")]],
      {
        show_ra_download_modal(
  ...
  # Bind all detailed ShinyCell tabs for one dataset prefix. The UI builders in
  # dataset_tab_builders.R generate IDs with the same prefix, so this function
  # can reuse the same output and input wiring across all built-in datasets.
  bind_shinycell_dataset <- function(prefix,
    ...
    # Legend toggles are optional per tab; default to visible when the UI has not
    # yet reported a value.
    ...
    # Remove guides from ggplot outputs when a tab-specific legend toggle is off.
    ...
    # Choose a robust y-axis value for violin/boxplot tabs by preferring numeric
    # metadata, then configured defaults, then gene names.
    ...
    # Refresh one gene selectize control with the current dataset's canonical
    # gene names.
    ...
    # Rebuild violin y-axis choices after lazy dataset assets become available.
    ...
    # Initialize expensive selectize choices only for the visible tab so opening
    # one dataset does not populate every control in every hidden pane.
    ...
    # Queue active-tab input initialization until the UI output has flushed.
    queue_active_tab_input_refresh <- function(tab_value = isolate(input$mainTabs)) {
      session$onFlushed(function() {
        return(update_active_tab_inputs(tab_value))
      }, once = TRUE)
      return(invisible(NULL))
    }
    ...
    bind_subset_controls <- function(block) {
      ...
      return(observeEvent(input[[make_id(paste0(block, "sub1all"))]], {
        conf <- get_conf()
        selected <- input[[make_id(paste0(block, "sub1"))]]
        sub <- strsplit(conf[UI == selected]$fID, "\\|")[[1]]
        updateCheckboxGroupInput(
    ...
    # Reuse the same modal shell for every generated dataset plot download menu.
    bind_download_modal <- function(button_suffix,
                                    entries_builder,
                                    intro = "Choose a figure, file type, and export size.") {
      return(observeEvent(input[[make_id(button_suffix)]],
        {
          show_ra_download_modal(
    ...
    # CSV upload state for the Multiple GeneExpr tab. This upload accepts only a
    # single GeneName column and then pushes valid names into the selectize input.
    ...
    output[[make_id("m1upload_status")]] <- renderUI({
      ...
      alert_class <- switch(tolower(status$type),
    ...
    # Aggregate the selected multi-gene list once so the plot and data table use
    # the same validated gene set.
    m1_data <- reactive({
      data <- scDRnumMulti(
        get_conf(),
        get_meta(),
  ...
  # Dataset tabs share the same builders and server bindings; these definitions
  # describe the per-tab UI factory for each dataset.
  ...
  # Dataset specs connect a ShinyCell prefix to its lazy data bindings and HDF5
  # expression matrix.
  dataset_specs <- list(
    sc3 = list(
      prefix = "sc3",
      ...
      gexpr_path = sc_data_candidates("sc3gexpr.h5")
    ),
    sc4 = list(
      prefix = "sc4",
      ...
      gexpr_path = sc_data_candidates("sc4gexpr.h5")
    ),
    sc5 = list(
      prefix = "sc5",
      ...
      gexpr_path = sc_data_candidates("sc5gexpr.h5")
    ),
    sc6 = list(
      prefix = "sc6",
      ...
      gexpr_path = sc_data_candidates("sc6gexpr.h5")
    ),
    sc7 = list(
      prefix = "sc7",
      ...
      gexpr_path = sc_data_candidates("sc7gexpr.h5")
    )
  )

  # Each dataset is initialized only after the user opens one of its registered
  # hidden tabs. This keeps startup memory lower and avoids building hidden UI
  # for all datasets at once.
  ...
  # Convert a registered tab value into the renderUI output that will host the
  # lazily-built tab body.
  dataset_tab_output_id <- function(tab_value) {
    return(paste0("lazy_", tab_value, "_body"))
  }

  # Extract the dataset prefix from tab IDs such as sc3_cellinfo_gene.
  ...
  # Placeholder displayed while the first-open dataset initialization is running.
  ...
  # Register a renderUI shell for every hidden dataset tab. The shell displays a
  # loading panel until ensure_dataset_initialized() has bound the server outputs.
  ...
  # Bind dataset outputs and reveal the lazily-built UI on first use.
  ...
  # ---------------------------------------------------------------------------
  # Spermatogenesis interactive table
  # ---------------------------------------------------------------------------

  # Build the clickable SVG hitbox layout that sits on top of the static
  # spermatogenesis table art.
  make_layout <- function() {
    # Constants retained from the original 900px-wide absolute-button layout.
    origin_x <- 37
    origin_y <- 29
    cell_w <- 62
    cell_h <- 61
  ...
  # Render the static table art plus transparent clickable rectangles.
  ...
  # When any modal closes, remove the active spermatogenesis highlight and unlock
  # future table clicks.
  ...
  # Shared interactive-table settings and helpers.
  DEFAULT_EXPR_THRESHOLD <- 0.25

  `%||%` <- function(x, y) if (is.null(x)) y else x

  # Format expression values consistently for UI labels and export subtitles.
  format_spg_expr <- function(x) {
    return(formatC(x, format = "f", digits = 3))
  }

  # Keep the expression threshold numeric and non-negative.
  ...
  # Scale a vector to [0, 1] for heatmap overlays, treating flat vectors as the
  # strongest shade so selected cells remain visible.
  minmax01 <- function(x) {
  ...
  # Clamp a numeric vector to the [0, 1] interval.
  clamp01 <- function(x) {
    return(pmax(0, pmin(1, x)))
  }

  # Mix an overlay color toward white to keep text and borders legible.
  mix_hex_with_white <- function(hex, weight) {
    rgb <- grDevices::col2rgb(hex)
    mixed <- rgb[, 1] * clamp01(weight) + 255 * (1 - clamp01(weight))
    return(grDevices::rgb(mixed[1], mixed[2], mixed[3], maxColorValue = 255))
  }

  # Interpolate a heatmap color from a numeric [0, 1] value.
  interpolate_hex <- function(scaled, stops) {
    t <- clamp01(scaled)
    rgb <- grDevices::colorRamp(stops)(t)
    return(grDevices::rgb(rgb[, 1], rgb[, 2], rgb[, 3], maxColorValue = 255))
  }

  # Return fill, stroke, and alpha values for a table-tile heat overlay in the
  # active theme.
  spg_heat_visual <- function(base_color, scaled, dark_theme = FALSE) {
    scaled <- if (is.finite(scaled)) clamp01(scaled) else NA_real_
    if (isTRUE(dark_theme)) {
      fill_color <- if (is.finite(scaled)) {
        interpolate_hex(scaled, c("#38bdf8", "#22d3ee", "#facc15"))
      } else {
        "#38bdf8"
      }
      return(list(
        fill = fill_color,
        stroke = mix_hex_with_white(fill_color, 0.82),
        alpha = if (is.finite(scaled)) 0.48 + 0.34 * scaled else 0.62
      ))
    }

    mix_weight <- if (is.finite(scaled)) 0.35 + 0.65 * scaled else 0.75
    return(list(
      fill = mix_hex_with_white(base_color, mix_weight),
      stroke = base_color,
      alpha = if (is.finite(scaled)) 0.65 + 0.25 * scaled else 0.75
    ))
  }

  # Empty data frame used before a cell modal has populated its gene table.
  ...
  # Load and cache the mouse gene-name to Ensembl-ID mapping used in modal CSVs.
  ...
  # Add Ensembl IDs to a modal gene table, using "Not found" for unmapped genes.
  ...
  spg_heat_overlay_state <- reactiveVal(NULL)
  spg_last_searched_gene <- reactiveVal(NULL)

  # Current expression threshold used by both cell modals and gene heat overlays.
  spg_expr_threshold <- reactive({
    sanitize_spg_expr_threshold(input$spg_expr_threshold)
  })

  # Explain the current gene range and threshold next to the heatmap controls.
  ...
  # Download the full modal gene table, not just the on-screen preview rows.
  output$spg_modal_genes_csv <- downloadHandler(
    filename = function() {
      ...
      return(paste0("genes_above_threshold_", suffix, ".csv"))
    },
    content = function(file) {
      full_df <- spg_modal_table_full()
      return(utils::write.csv(full_df, file, row.names = FALSE))
    }
  )

  # Clear all client-side table highlights and, optionally, the stored search
  # state.
  ...
  # Search the precomputed average-expression matrix for one gene, highlight
  # matching cell tiles, and store overlay data for exports.
  run_spg_gene_search <- function(query = NULL, show_notifications = TRUE) {
    ...
    overlay_df <- data.frame(
      btn_id = as.character(match_ids),
      color = as.character(cols),
      value = as.numeric(expr_vals),
      rank = seq_along(match_ids),
  ...
  # Draw readable labels on exported heatmap tiles by first painting a small
  # outline around the text.
  draw_spg_outlined_text <- function(label, x, y, dark_theme = FALSE, fontsize = 8) {
    outline <- if (isTRUE(dark_theme)) "#020617" else "#ffffff"
    fill <- if (isTRUE(dark_theme)) "#f8fafc" else "#111827"
    offsets <- expand.grid(dx = c(-0.7, 0, 0.7), dy = c(-0.7, 0, 0.7))
    offsets <- offsets[offsets$dx != 0 | offsets$dy != 0, , drop = FALSE]
    for (i in seq_len(nrow(offsets))) {
      grid::grid.text(
        label,
        x = grid::unit(x, "native") + grid::unit(offsets$dx[i], "pt"),
        y = grid::unit(y, "native") + grid::unit(offsets$dy[i], "pt"),
        gp = grid::gpar(col = outline, fontsize = fontsize, fontface = "bold")
      )
    }
    return(grid::grid.text(
      ...
      gp = grid::gpar(col = fill, fontsize = fontsize, fontface = "bold")
    ))
  }

  # Build the default export filename from the current gene overlay when present.
  spg_export_default_stem <- function() {
    state <- spg_heat_overlay_state()
    if (is.null(state) || !nzchar(state$gene_name %||% "")) {
      return("spermatogenesis_table")
    }
    gene_stem <- gsub("[^A-Za-z0-9_-]+", "_", state$gene_name)
    return(paste0("spermatogenesis_table_", gene_stem))
  }

  # Render the spermatogenesis table art and optional heat overlay onto the
  # active graphics device.
  draw_spg_table_export <- function(overlay_state = NULL, dark_theme = FALSE) {
    view_w <- 900
    view_h <- 486
    bg_color <- if (isTRUE(dark_theme)) "#020617" else "#ffffff"
    text_color <- if (isTRUE(dark_theme)) "#e2e8f0" else "#0f172a"
    subtext_color <- if (isTRUE(dark_theme)) "#cbd5e1" else "#475569"

    background_name <- if (isTRUE(dark_theme)) "interactiveTable_dark.png" else "interactiveTable.png"
    background_path <- sc_www_path(background_name, app_dir = app_dir)
    if (!file.exists(background_path)) {
      background_path <- sc_www_path("interactiveTable.png", app_dir = app_dir)
    }
    if (!file.exists(background_path)) {
      stop("Unable to find interactiveTable.png for spermatogenesis table export.", call. = FALSE)
    }

    has_overlay <- !is.null(overlay_state) &&
      is.data.frame(overlay_state$overlay) &&
      nrow(overlay_state$overlay) > 0

    title <- if (has_overlay) {
      sprintf(
        "Spermatogenesis table heatmap: %s",
        overlay_state$gene_name %||% "selected gene"
      )
    } else {
      "Spermatogenesis table"
    }
    subtitle <- if (has_overlay) {
      sprintf(
        "Mean RNA expression per cell tile; threshold > %s; %d highlighted tile(s).",
        format_spg_expr(overlay_state$threshold %||% DEFAULT_EXPR_THRESHOLD),
        nrow(overlay_state$overlay)
      )
    } else {
      "No gene overlay selected. Search a gene first to include heatmap colors."
    }
    ...
    grid::grid.text(
      title,
      x = grid::unit(0.25, "in"),
      y = grid::unit(1, "npc") - grid::unit(0.20, "in"),
      just = c("left", "top"),
      gp = grid::gpar(col = text_color, fontsize = 13, fontface = "bold")
    )
    grid::grid.text(
      subtitle,
      x = grid::unit(0.25, "in"),
      y = grid::unit(1, "npc") - grid::unit(0.43, "in"),
      just = c("left", "top"),
      gp = grid::gpar(col = subtext_color, fontsize = 8.5)
    )

    table_viewport <- grid::viewport(
      y = grid::unit(0, "npc"),
      height = grid::unit(1, "npc") - grid::unit(0.68, "in"),
      just = "bottom",
      clip = "on"
    )
    grid::pushViewport(table_viewport)
    grid::grid.raster(
    ...
    if (has_overlay) {
      grid::pushViewport(grid::viewport(xscale = c(0, view_w), yscale = c(0, view_h)))
      overlay <- overlay_state$overlay
      layout_idx <- match(overlay$btn_id, buttons_layout$btn_id)
      keep <- !is.na(layout_idx)
      overlay <- overlay[keep, , drop = FALSE]
      layout_rows <- buttons_layout[layout_idx[keep], , drop = FALSE]

      for (i in seq_len(nrow(overlay))) {
        visual <- spg_heat_visual(
          base_color = overlay$color[i],
          scaled = overlay$scaled[i],
          dark_theme = dark_theme
        )
        center_x <- layout_rows$x[i] + layout_rows$w[i] / 2
        center_y <- view_h - (layout_rows$y[i] + layout_rows$h[i] / 2)

        grid::grid.roundrect(
          x = grid::unit(center_x, "native"),
          y = grid::unit(center_y, "native"),
          width = grid::unit(layout_rows$w[i], "native"),
          height = grid::unit(layout_rows$h[i], "native"),
          r = grid::unit(3, "pt"),
          gp = grid::gpar(
            fill = scales::alpha(visual$fill, visual$alpha),
            col = visual$stroke,
            lwd = 1.6
          )
        )

        draw_spg_outlined_text(
          label = formatC(overlay$value[i], format = "f", digits = 2),
          x = center_x,
          y = center_y,
          dark_theme = dark_theme
        )
  ...
  # Open the requested graphics device and draw the spermatogenesis table export.
  render_spg_table_export <- function(file, device = c("png", "pdf")) {
    device <- match.arg(device)
    width <- download_dimension("spg_table_w", 12)
    height <- download_dimension("spg_table_h", 6.9)
    dark_theme <- isolate(is_dark_mode())
    background <- if (isTRUE(dark_theme)) "#020617" else "#ffffff"
    overlay_state <- isolate(spg_heat_overlay_state())
    ...
    on.exit(grDevices::dev.off(), add = TRUE)
    return(draw_spg_table_export(overlay_state = overlay_state, dark_theme = dark_theme))
  }

  observeEvent(input$spg_downloads_open,
    {
      overlay_state <- spg_heat_overlay_state()
      has_overlay <- !is.null(overlay_state) &&
        is.data.frame(overlay_state$overlay) &&
        nrow(overlay_state$overlay) > 0
      intro <- if (has_overlay) {
        sprintf(
          "Export the spermatogenesis table with the current %s heatmap overlay.",
          overlay_state$gene_name %||% "gene"
        )
      } else {
        "Export the plain spermatogenesis table. Search a gene first to include the heatmap overlay."
      }

      show_ra_download_modal(
        entries = ra_download_entry(
          pdf_id = "spg_table_pdf",
          png_id = "spg_table_png",
          height_id = "spg_table_h",
          width_id = "spg_table_w",
          height_value = 6.9,
          width_value = 12,
          title = "Spermatogenesis table",
          filename_id = "spg_table_name"
        ),
        intro = intro
      )
    },
  ...
  output$spg_table_pdf <- downloadHandler(
    filename = function() {
      return(resolve_download_filename(input$spg_table_name, spg_export_default_stem(), ".pdf"))
    },
    content = function(file) {
      return(render_spg_table_export(file, device = "pdf"))
    }
  )

  output$spg_table_png <- downloadHandler(
    filename = function() {
      return(resolve_download_filename(input$spg_table_name, spg_export_default_stem(), ".png"))
    },
    content = function(file) {
      return(render_spg_table_export(file, device = "png"))
    }
  )


  # Build and show the gene-expression modal for one clicked spermatogenesis
  # table cell.
  ...
  # Close button inside the custom spermatogenesis modal reports the active
  # button ID so its highlight can be removed deterministically.
  ...
  # Single handler for any spermatogenesis cell click.
  observeEvent(input$btn_click,
    {
      if (isTRUE(spg_modal_open()) || isTRUE(app_modal_open())) {
  ...
  # Recalculate the overlay when a searched gene remains active and the threshold
  # changes.
  ...
  # Re-send heatmap colors after theme changes so exported and on-screen overlay
  # colors stay legible in the active theme.
  ...
  # ============================
  # PUBLICATION/REUSABLE BUILDERS
  # ============================

  # Build Figure 5A as a ggplot from precomputed RA average-expression and
  # percent-expression matrices.
  make_fig5A <- function(pub_theme = FALSE, dark_theme = FALSE) {
    ...
    # Row-wise z-scaling matches the publication-style dotplot color encoding.
    scale_rows <- function(mat) {
      return(t(apply(mat, 1, function(x) {
    ...
    # Match the publication Figure 5A palette from the RA signaling reference.
    publication_rd_bu <- rev(
      grDevices::colorRampPalette(RColorBrewer::brewer.pal(9, "RdBu"))(100)
    )
  ...
  # Build Figure 5C as either the publication patchwork layout or a single
  # faceted ggplot from precomputed RA line means.
  make_fig5C <- function(pub_theme = TRUE, dark_theme = FALSE, pub_layout = TRUE) {
    ...
    stage_label_short <- function(values) {
      return(sub("\\s*\\(.*\\)", "", as.character(values)))
    }
    ...
    # Scale expression within each gene/cell-type trajectory so stage trends are
    ...
    # Build one row of the compact publication layout.
    build_row_plot <- function(row_name, show_x = FALSE) {
      ...
      return(ggplot(df_row, aes(
        x = sample, y = scaled_expr, color = gene, group = gene,
        text = sprintf(
    ...
    return(ggplot(df_rescaled, aes(
      x = sample, y = scaled_expr, color = gene, group = gene,
      text = sprintf(
        "Gene: %s<br>Cell: %s<br>Stage: %s<br>Scaled: %.2f<br>Mean: %.2f",
  ...
  # Debounce RA dotplot controls so rapid selector changes produce one render.
  ...
  # Event reactive isolates Figure 5A rendering behind an explicit tick so
  # control changes can be throttled and first-open retries can repaint it.
  ...
  # Figure 5C lineplot defaults are set in ensure_line_assets().

  # Debounce RA lineplot controls so editing all three gene rows does not trigger
  # a render for every intermediate selection.
  ...
  # Event reactive isolates Figure 5C rendering behind an explicit tick.
  ...
  # Ensure communication scores are available even if this file is sourced in an
  # unusual order during local development.
  ...
  # Seed the ligand-receptor selector with publication defaults when available.
  ...
  observeEvent(input$mainTabs,
    {
      if (!identical(input$mainTabs, "cell2cell_heatmaps")) {
        return()
      }
      ensure_ccc_defaults()
  ...
  observe({
    ...
    if (attempts_left <= 0L || !identical(input$mainTabs, "cell2cell_heatmaps")) {
      return()
    }
    invalidateLater(1400, session)
    ensure_ccc_defaults()
  ...
  # Debounce heatmap control changes before incrementing the explicit render tick.
  ...
  # Generate the interactive Plotly heatmap from the selected ligand-receptor
  # pairs.
  ccc_heatmap_event <- eventReactive(ccc_tick(),
    {
      ...
      # Safer subsetting
      filtered <- communication_score[
        communication_score$lr_pair %in% selected,
  ...
  # Static ggplot builder for publication downloads, separate from the Plotly
  # widget used on screen.
  make_fig6D_static <- function(selected_pairs, dark_theme = FALSE) {
    ...
    filtered <- communication_score[
      communication_score$lr_pair %in% selected_pairs,
      c("lr_pair", "X_DARK_score", "X_PALE_score", "X_PALE2WEAK_score", "X_WEAK2STRONG_score")
    ]
    ...
    return(ggplot2::ggplot(df_long, ggplot2::aes(x = stage, y = lr_pair, fill = score)) +
      ggplot2::geom_tile() +
  ...
  return(output$ccc_png <- downloadHandler(
